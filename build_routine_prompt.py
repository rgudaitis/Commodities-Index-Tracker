...
with open(os.path.join(HERE, "sandbox_fetch_data.py"), encoding="utf-8") as f:
    fetch_src = f.read()
...
BACKGROUND — why fetch_data.py pulls from GitHub instead of Yahoo Finance/BLS directly:
This sandbox's outbound network proxy only allows a small allowlist of hosts (confirmed via diagnostic: only api.github.com reachable; Yahoo Finance, BLS, and every other external host tested returned "Tunnel connection failed: 403 Forbidden"). A separate GitHub Actions workflow (in the public repo rgudaitis/Commodities-Index-Tracker, running on GitHub's own unrestricted runners) fetches the real commodity data daily and commits it as data.json. fetch_data.py below just reads that file back via api.github.com, which this sandbox CAN reach. Do not attempt to fetch Yahoo Finance or BLS URLs directly from this sandbox (via requests, WebFetch, or otherwise) — both approaches have been tried and don't work reliably from here. If data.json looks stale (fetched_at more than ~36 hours old), that means the GitHub Actions workflow itself needs attention — that's a separate system from this routine, don't try to work around it here.
...
Then run it: `python3 fetch_data.py`. This fetches today's already-computed commodity snapshot from data.json in the rgudaitis/Commodities-Index-Tracker GitHub repo (via the GitHub API, the one host this sandbox can reach), then merges it into history.json (removing any existing entry for today's date first, so re-runs are safe).
...
If ANY of (a)/(b)/(c) fail, OR if STEP 2's fetch_data.py, STEP 3's build_dashboard.py, or STEP 4's Artifact publish raised an exception/error at any point, treat this run as FAILED. In that case, call the PushNotification tool with status="proactive" and a message under 200 characters stating what failed, e.g. "Commodities index: today's refresh failed fetching data.json from GitHub, dashboard not updated — check {ARTIFACT_URL}" or "Commodities index: artifact republish didn't verify — today's snapshot missing from live page." Be specific about which step failed so it's actionable. Note: per STEP 3's build_dashboard.py, a source outage no longer blanks the whole dashboard (it falls back to the last good value per commodity, marked as stale) — the PushNotification is still required on this condition because a fallback masks the outage visually but someone still needs to know the data is stale and why.
...
STEP 5 — Report back briefly (as your final text output): today's composite Day/YTD/YoY % changes, how many of the 8 commodities fetched cleanly vs fell back to stale data, and confirm the artifact URL was updated successfully (not a new URL) and that STEP 4.5 verification passed. If STEP 4.5 failed and you already sent a PushNotification, say so here too.
...
- The actual Yahoo/BLS fetching happens in a separate GitHub Actions workflow (rgudaitis/Commodities-Index-Tracker, .github/workflows/fetch-data.yml, cron 10:45 UTC daily) — this routine only consumes its output (data.json). If data.json is missing or stale, that workflow needs debugging, not this routine's fetch_data.py.
- Don't invent, estimate, or hallucinate any price/index/YTD/YoY values. If data.json itself has a per-commodity error (from the GitHub Actions run), or if fetching data.json fails entirely, leave those error fields as-is — build_dashboard.py will fall back to the last known-good value for display and mark it stale rather than guessing a number.
- Do not modify the Python script logic — copy it verbatim. The scripts already handle duplicate-day de-duplication and stale-data fallback.
